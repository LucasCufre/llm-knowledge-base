...
See [Inbox README](00-inbox/README.md) for examples and best practices.
...
- **Cursor** - Uses `.cursorrules` (linked to CLAUDE.md)
- **Cline** - Uses `.clinerules` (linked to CLAUDE.md)
- **Windsurf** - Uses `.windsurfrules` (linked to CLAUDE.md)
- **Continue** - Uses `.continue/instructions.md` (linked to CLAUDE.md)
- **Gemini** - Uses `.gemini/instructions.md` (linked to CLAUDE.md)

A root-level `instructions.md` link is also provided for generic tool compatibility.

All tools share the same instructions, ensuring consistent behavior across your team's preferred AI assistants.

### Platform Setup

On **macOS/Linux**, symlinks work out of the box after cloning.

On **Windows**, git checks out symlinks as plain text files by default. Run the setup script after cloning to create proper file copies:
...
If you update `CLAUDE.md`, re-run the script to sync changes to all tool config files.
